...
try:
    # First print content type header
    ...
    # Use new name if provided, otherwise use original filename
    final_name = (new_name if new_name else Path(original_filename).stem) + ext
    new_filename = os.path.join(upload_dir, final_name)
    
    # Save file
    with open(new_filename, 'wb') as f:
        f.write(file_data)
    
    print(f"File uploaded successfully as {final_name}")
